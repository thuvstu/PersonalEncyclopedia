...
| 15 | bookmark.html取込 / ポモドーロタイマー | 未着手 | コード0件 | §2.5・§2.6 |

> 要約: **達成ゼロ・部分13・未着手2**。骨格 (entry統一型・FTS+ベクトル検索・SM-2/FSRS・承認制・SAFバックアップ) は構想を超えて実装された一方、構想の顔である「可視化・自動リンク・リッチ文書・通知・取込」の体験層が部分止まり。これが本書の主題である。
...
`pomodoro|Pomodoro|ポモドーロ` は `RealityTasks.md:11` の1件のみ。`Timer` はクイズ画面の表示用アイコンのみ。25分/5分サイクル・休憩管理・集中統計なし。類似物はToDoのタイムボックスcountdownのみ。

### §2.6 bookmark.html取込 → 未着手

`bookmark|hoarder|linkwarden|Netscape` は `RealityTasks.md:10` の1件のみ。`ImportPipeline` の5経路 (CSV/MD/JSON/URLリスト/Notion) にHTMLパーサなし。`ImportScreen.kt:30-41` に選択肢なし。1行1URLリストなら取込可だが、数百件ブックマークのフォルダ・タグ・既読の一括移行は不可。
