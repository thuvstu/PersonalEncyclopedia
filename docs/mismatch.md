...
| 2 | 文書中の単語検出→自動ハイパーリンク→定義ジャンプ | 部分→**改善（2026-09-06）** | 詳細/Wikiで発動(wt25/27)、索引共有(wt44)、表記揺れ・読み・別名(wt46)。残: 編集中サジェスト | §1.2 |
...
~~5万件実用性も未検証: VM側5000件打切りとProvider側50000件の不整合、増分更新なし~~ → **解消（2026-09-06・walkthrough44）**: `AutoLinkerProvider` が `COUNT-MAX(updatedAt)` 指紋で自動再構築、詳細画面も同じ共有キャッシュを使用(上限なし・タイトル列のみ読込)。

~~完全一致Trie (表記揺れ・読み仮名非対応)~~ → **改善（2026-09-06・walkthrough46）**: 全半角・大小文字・半角カナを1:1正規化して照合、定義の読み・人物の別名/fullName・組織/場所/出来事の正式名を `EntryDao.getAllAliasForms` で同じTrieに登録(`[[正式タイトル|原文]]` で埋込)。単語/カタカナ境界・ひらがな埋没抑制・語末長音吸収で誤検出を抑えた(シード232テキストで新旧差分0)。残: 異体字の自動吸収(別名登録で代替)、ひらがな⇄カタカナ、5万件実測。

構想の「検出次第自動生成」のうち閲覧時装飾は配線済み(wt25/27/44/46)。エディタ入力中サジェストは未着手。
