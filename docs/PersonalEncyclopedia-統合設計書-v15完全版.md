...
**バージョン:** 15.0 (Task & External Integration Edition)
**作成日:** 2026-08-17
**ベース:** v14.0(Researched & Expanded Edition) + タスク管理(パーキンソンの法則対抗)・StudyPlus連携・SQL Explorer・編集履歴を新規追加
**統合元:** (v14までと同じ全案) + Studyplus-Android-SDK(実SDK調査) + SQLiPKM(AI Studio生成PKM、参考採用)
**ステータス:** 確定版(Phase 0〜4実装済み・実機ビルド確認済み・Phase 5以降の実装仕様として使用)

## v15.0での主な変更点(v14.0からの差分)

| 変更 | 内容 |
|---|---|
| §5.9(新設) | タスク管理・編集履歴のデータモデル(`task`, `task_time_log`, `entry_history`) |
| §8.10(新設) | タスクエンジン: パーキンソンの法則対抗ロジック(見積もり記録・先延ばし上限・強制対峙) |
| §7.8(新設) | StudyPlus連携(公式Android SDK調査に基づく実装仕様) |
| §11.11-11.13(新設) | ToDo画面・SQL Explorer画面(読み取り専用強制)・編集履歴UI |
| §4.1 | Studyplus-Android-SDK(JitPack経由)を技術スタックに追加 |
| §15.3(新設) | SQLiPKM(AI Studio生成)の評価と採用箇所の記録 |
...
| 外部連携(学習記録) | `com.github.studyplus:Studyplus-Android-SDK`(JitPack) | §7.8参照。要件: Android 6+、端末にStudyplusアプリ7.0+がインストール済みであること |
...
| Studyplus-Android-SDK | 4.0.2(JitPack配布) | §7.8参照。`coreLibraryDesugaringEnabled true`が必須 |

**Studyplus SDK導入設定**(実SDK確認済み):

```kotlin
// settings.gradle.kts のrepositoriesにJitPackを追加
...
### 5.9 タスク管理・編集履歴のデータモデル(v15.0で新設)

#### 5.9.1 タスク管理(パーキンソンの法則対抗)

「見積もりと締切を厳格に扱う」という要件のため、既存のプラグイン的な軽さではなく専用テーブルとする。`entry`とは独立させ(タスクは知識ではなく行動の管理対象)、必要に応じて`linkedEntryId`/`linkedTopicId`で知識側と紐付ける。

```kotlin
@Entity(tableName = "task", indices = [Index("status"), Index("deadlineAt")])
...
@Entity(tableName = "task_time_log")
...
#### 5.9.2 編集履歴(SQLiPKMより採用)

SQLiPKMの`NoteHistoryEntity`が単一Note型専用だったのに対し、本設計はentry統一型(§5.1)に合わせて**全13型+Wiki記事に共通で使える汎用スナップショット**として一般化する。

```kotlin
@Entity(tableName = "entry_history", indices = [Index("entryId"), Index("recordedAt")])
data class EntryHistoryEntity(
    @PrimaryKey val id: String = UUID.randomUUID().toString(),
    val entryId: String,
    val recordedAt: Long = System.currentTimeMillis(),
    val titleSnapshot: String,
    val contentSnapshot: String?,       // entry.contentのスナップショット(型固有拡張フィールドは対象外、§5.3のcontentのみ)
    val changeSummary: String = "",     // AI生成 or 空欄可
    val charCountDelta: Int = 0
)
```

型固有の拡張フィールド(`entry_definition.definition`等)まで含めた完全なスナップショットは、13型ぶんの差分ロジックが必要になり実装原則§2.5(既存コードへの影響を最小化)に反するため、**v15.0では`entry.content`(ユーザー注釈)のみを対象とする**。型固有フィールドの履歴化は需要が明確になった場合の拡張候補とする(§15参照)。
...
### 7.8 StudyPlus連携(v15.0で新設・公式Android SDK実物調査に基づく)

タスク完了時の学習時間を、外部の学習記録SNS「Studyplus」へ自動投稿する。公式SDK(`Studyplus-Android-SDK` 4.0.2)を実際に確認した結果、当初想定と異なる制約が判明したため、それを踏まえた設計とする。

#### 7.8.1 SDKの実際の仕様(確認済み)

```kotlin
class Studyplus(context: Context, consumerKey: String, consumerSecret: String) {
    fun isAuthenticated(): Boolean
    fun startAuth(activity: Activity, requestCode: Int)   // Studyplusアプリへ画面遷移してOAuth認証
    fun setAuthResult(data: Intent)                        // onActivityResultで呼ぶ
    fun cancelAuth()
    fun postRecord(record: StudyRecord, callback: PostCallback)
}

data class StudyRecord(
    val duration: Int,                  // 学習時間(秒)。最大86400(24h)
    val amount: StudyRecordAmount? = null,  // 学習量(ページ数等、任意)
...
**判明した重要な制約**:
1. **端末にStudyplusアプリ(7.0+)のインストールが必須**。`startAuth()`はアプリ間Intent遷移で認証するため、未インストール時は`ActivityNotFoundException`が発生する(READMEのサンプルコードで実際にcatchしている)。Webベースのフォールバック認証はSDK内に存在しない
2. **`StudyRecord`に科目/教材を指定するパラメータが無い**。投稿する学習記録は、Studyplus側で事前に紐付けられた「登録教材」単位に固定される想定になっている。つまり**タスクごとに異なる科目としてStudyplus側で振り分けることはできない**。トピック情報は`comment`欄に文字列として含める形で代替する
3. `consumerKey`/`consumerSecret`の取得には、Studyplus側への教材アプリ開発者としての登録が必要。ユーザーの理解通り、**まずアプリを完成させてから申請する**順序で問題ない(申請中・未承認の間はStudyPlus連携をOFFにしたまま他機能の開発を進められる設計にしておく)

#### 7.8.2 実装方針

```kotlin
// integration/StudyPlusClient.kt
class StudyPlusClient(context: Context, settings: Settings) {
    private val instance: Studyplus? = settings.studyPlusConsumerKey?.let { key ->
        Studyplus(context, key, settings.studyPlusConsumerSecret ?: "")
    }

    fun isAvailable(): Boolean = instance != null

    fun startAuth(activity: Activity, requestCode: Int): Boolean = try {
        instance?.startAuth(activity, requestCode)
        true
    } catch (e: ActivityNotFoundException) {
        false  // Studyplusアプリ未インストール。設定画面で「Studyplusアプリが必要です」と案内する
    }

    suspend fun syncTaskTimeLog(log: TaskTimeLogEntity, task: TaskEntity, topicName: String?) {
        if (log.studyPlusSynced || log.endedAt == null) return
        val durationSec = ((log.endedAt - log.startedAt) / 1000).toInt().coerceAtMost(86400)
        val comment = listOfNotNull(task.title, topicName).joinToString(" / ")
        val record = StudyRecord(duration = durationSec, comment = comment)
        instance?.postRecord(record, object : PostCallback {
            override fun onSuccess() { markSynced(log.id) }
            override fun onFailure(e: StudyplusError) { /* 次回同期時に再試行。エラーはAppLoggerへ */ }
        })
    }
}
```

- §9.2の拡張性設計(Interfaceパターン)に倣い、`StudyPlusClient`は§11.11のToDo画面・タスク完了処理からのみ呼ばれる形にし、コア学習エンジン(§8)には依存を持ち込まない
- 認証状態・連携ON/OFFは§11.10設定画面で管理する。未認証時はタスク完了時の同期を静かにスキップし(エラー扱いしない)、`task_time_log.studyPlusSynced`が`false`のまま溜まっている件数を設定画面に表示して、後から一括同期できるようにする
...
### 8.10 タスクエンジン: パーキンソンの法則対抗ロジック(v15.0で新設)

「作業は与えられた時間をすべて満たすまで膨張する」というパーキンソンの法則に対抗するため、単なるリマインダー付きToDoではなく、**見積もりの自己申告を記録し続けること**と**先延ばしを無限に握りつぶせなくすること**の2本柱で設計する。

#### 8.10.1 見積もり記録と乖離の可視化

```kotlin
class TaskEngine(private val taskDao: TaskDao, private val timeLogDao: TaskTimeLogDao) {
    // タスク作成: estimatedMinutesの入力を必須とする(UI側でバリデーション)
    suspend fun startTask(taskId: String) {
        timeLogDao.insert(TaskTimeLogEntity(taskId = taskId, startedAt = System.currentTimeMillis(), endedAt = null))
        taskDao.updateStatus(taskId, "in_progress")
    }

    suspend fun completeTask(taskId: String) {
        val log = timeLogDao.getOpenLogFor(taskId) ?: return
        timeLogDao.close(log.id, endedAt = System.currentTimeMillis())
        taskDao.updateStatus(taskId, "done", completedAt = System.currentTimeMillis())
    }

    // 見積もりと実績の乖離を集計(ダッシュボード§11.11で「あなたは見積もりを平均◯%過小評価しています」と表示)
    suspend fun estimationBiasReport(): EstimationBias {
        val completed = taskDao.getCompletedWithLogs()
        val ratios = completed.map { (it.actualMinutes.toDouble() / it.estimatedMinutes).coerceAtLeast(0.01) }
        return EstimationBias(averageRatio = ratios.average(), sampleSize = ratios.size)
    }
}
```
...
静かな先延ばしの繰り返しこそがパーキンソンの法則の温床であるため、**一定回数を超えたら選択を強制する**。
...
suspend fun postponeTask(taskId: String, newDeadlineAt: Long): PostponeResult {
    ...
    return if (newCount > MAX_SILENT_POSTPONES) {  // 既定3回
        // ここで静かな先延ばしを許さず、呼び出し元(UI)に強制選択を要求する
        PostponeResult.RequireForcedChoice(taskId)  // 「今日中に終わらせる」か「破棄(abandoned)」の二択画面を表示
    } else {
...
`RequireForcedChoice`を受け取ったUI(§11.11)は、通常のダイアログでは閉じられない(バックボタン無効化)強制選択画面を表示し、「今日の締切として確定する」または「タスクを破棄(status='abandoned')する」のいずれかを選ばせる。これにより「先延ばしを先延ばしする」という抜け道を塞ぐ。

#### 8.10.3 タイムボックス終了時の強制対峙

```kotlin
// WorkManagerのワンショットジョブで estimatedMinutes 経過時にトリガー
// スヌーズは最大2回までとし、それ以降は§8.10.2と同じ強制選択フローに合流させる
```

#### 8.10.4 進捗記録との統合

タスクの開始・完了・破棄は`progress_events`(§5.7)に`entityType='task'`として記録し、既存の学習統計画面(§11.3)にタスク完遂率も表示できるようにする。既存のスコア/攻略度エンジン(§8.1)とは独立した指標として扱い、無理に統合しない(タスク管理は行動管理、クイズは知識定着という別ドメインであるため)。
...
| 外部連携 | StudyPlus連携ON/OFF・認証状態(§7.8)、consumer key/secret設定欄(申請承認後に入力)、未同期件数の表示 |

### 11.11 ToDo画面(v15.0で新設・§5.9.1/§8.10のデータモデルに対応)

- **一覧**: `status`別(進行中/期限間近/完了/破棄)にセクション分けしたリスト。締切が近いほど視覚的に強調(色の変化。ゲーミフィケーション寄りの過度な演出は避ける)
- **タスク作成**: `estimatedMinutes`の入力を必須項目とし、未入力では保存ボタンを無効化する(§8.10.1の前提を画面レベルで担保)
- **実行中画面**: タイムボックスのカウントダウン表示。§8.10.3のタイムアウト時、通常の通知ではなく**アプリ内モーダル**で強制対峙を表示する(バックグラウンド時は通知経由でアプリを前面化させてから同モーダルを表示)
- **強制選択モーダル**(§8.10.2): `RequireForcedChoice`受領時に表示。「今日中に終わらせる」/「破棄する」の2択のみで、閉じるボタン・バックボタンでの回避を許さない
- **見積もり精度レポート**: `estimationBiasReport()`の結果をダッシュボード(§11.3)にカード表示(「直近30件の見積もり精度: 平均1.8倍」等)

### 11.12 SQL Explorer画面(v15.0で新設・SQLiPKMより採用、読み取り専用を強制)

SQLiPKMの発想を採用しつつ、確認で見つかった安全上の懸念(生SQLをそのまま実行し書き込み系ステートメントを防いでいない)に対応した設計とする。

```kotlin
class ReadOnlySqlExecutor(private val database: AppDatabase) {
    private val writeKeywords = setOf("INSERT", "UPDATE", "DELETE", "DROP", "ALTER", "CREATE", "REPLACE", "PRAGMA")

    suspend fun executeReadOnly(sql: String): SqlExecutionResult {
        val firstToken = sql.trim().split(Regex("\\s+")).firstOrNull()?.uppercase()
        if (firstToken != "SELECT" && firstToken != "WITH") {
            return SqlExecutionResult.error("読み取り専用です(SELECT/WITHのみ実行可能)")
        }
        // 加えて、Roomの読み取り専用コネクション(readableDatabaseではなく可能ならreadOnlyフラグ付き接続)を使う
        return runQuery(sql)
    }
}
```

キーワードチェックだけでは完全な防御にならない(コメントやサブクエリでの回避余地がある)ため、**可能であれば接続自体を読み取り専用モードで開く**二重の防御とする(SQLiteの`PRAGMA query_only = ON`をこの接続にだけ適用する等)。用途はデバッグ・DB管理画面(§11.3)からのみ到達可能とし、通常のユーザー導線には置かない。

- **スキーマブラウザ**: テーブル/ビュー一覧、`PRAGMA table_info`によるカラム一覧(SQLiPKMの`getTableSchemas()`を踏襲)
- **保存済みクエリ**: よく使うSELECTを名前付きで保存(`saved_query`テーブル、SQLiPKMの`SavedQueryEntity`を踏襲)
- **DB統計**: `journal_mode`/`page_count`/`integrity_check`等のPRAGMA値表示

### 11.13 編集履歴UI(v15.0で新設・§5.9.2のデータモデルに対応)

- エントリー詳細画面(§11.3)に「履歴」タブを追加。`entry_history`を時系列表示し、`changeSummary`と`charCountDelta`(+12文字 / -3文字等)を一覧化
- 過去のスナップショットをタップすると読み取り専用のプレビュー表示。**復元機能(過去バージョンへの巻き戻し)はv15.0のスコープ外**とし、閲覧のみに留める(復元は上書きの取り消し不能な操作であり、実装原則§2.5の「最小限の変更」から外れるため、需要が明確になってから別途設計する)
...
-     匿名/OAuth公開共有プラットフォーム(KnOS EX相当)を独立プロジェクトとして検討 →Wikiビルダーがその役割を担っている
-     ナビ/経路案内(txt.mdの「他サービス連携」構想の一部) →別サービスで実装は必要ない
...
### 15.3 参考採用: SQLiPKM(Google AI Studio・無指示生成)

Google AI Studioに指示なしで生成させたAndroidネイティブPKMアプリ(Room/Compose、38ファイル)。単一Note型+タグ+リンクという小規模構成で、13型CTIを持つ本設計より野心は小さいが、以下2点は実際にコードを確認した上で価値を認め、v15.0に採用した。

| 採用した要素 | 採用先 | 備考 |
|---|---|---|
| SQL Explorer(生SQL実行+スキーマブラウザ) | §11.12 | 元実装は書き込み系ステートメントへのガードが無く、そのままでは危険。読み取り専用強制を追加した上で採用 |
| ノート編集履歴(`changeSummary`+`charCountDelta`) | §5.9.2, §11.13 | 単一Note型専用だった実装を、entry統一型(13型共通)に一般化して採用 |

**採用しなかった理由**: 単一Note型のシンプルさは、本設計が既に卒業した段階(13型CTI・接続承認制・Hybrid Search)であり、全体構成としての乗り換えは後退にあたる。あくまで個別機能のアイデア源として部分採用に留めた。
...
### 16.5 現時点の到達点(v15.0で更新)

Phase 0〜4相当(entry統一型全13型・ホワイトボード・Wikipedia・FSRS・Ollama・接続承認制・コーチング等)が**実際にビルドされ、実機で動作することを確認済み**。GAP-1〜6(データ保護・セキュリティ・スレッドセーフ・和暦マスタ・マイグレーションテスト・Ktorルーティング分割)もすべて実コードで検証済み。これは過去7つの計画のうち6つが実装にすら着手できなかったことと比べると、明確な前進である。v12.0の撤回も、失敗ではなく「実装原則を実地で得るために必要だった1回」として位置づけてよい。§13のRound方式は、この教訓を具体的な作業手順にまで落とし込んだものであり、後は積み重ねるだけの状態にある。

一方で、GAP以降の複数回の実装(GAP一式・v14要素一式)が、いずれも実装原則§2.5の「1機能1ビルド」から外れた大きなバッチで行われ、結果的にはすべて整合していたという経緯も記録しておく。これは実装力の高さの証明であって、プロセスが安全である証明ではない。v15.0のタスク管理・StudyPlus連携・SQL Explorer・編集履歴についても、実装時は同じ規律(1機能1ビルド、既存コードを唯一の正とする)を適用することが望ましい。

---

*本書はこれまでの全7系統の学習支援システム計画(Supabase案・GAS/LearningSheet v25・LearningMasterMap・ALH Omni-Master v26/All-Specification v5.2・Personal Knowledge OS v10・KnOS EX・quizstudy/lumina_pkm/txt.md)に加え、実装で得た教訓(v12.0撤回の分析・報告書.md)、調査済みの最新ライブラリ・AIモデル情報、拡充したクイズバリエーション、開発者ガイド要件、実機ビルドで検証したGAP修正一式、タスク管理(パーキンソンの法則対抗)・StudyPlus連携(実SDK調査)・SQL Explorer・編集履歴(SQLiPKM参考採用)、および検討の上で不採用とした2案(MyBase・nextPKM.txt)を統合したv15.0版である。ベースはAndroidネイティブ最新版であるPersonalEncyclopediaとし、その「データ主権をAndroidに置く」というエコで壊れにくい骨格は、実装の紆余曲折を経てなお変更していない。「何を作るか」はv11.0〜v15.0の設計を継承・拡充し、「どう作るか」は§2.5の実装原則によって刷新し、「理解できるか」を§14.1の開発者ガイド要件によって新たに要求した。*
...
/design.mdと/docs/guide/は適宜実態に合わせて更新