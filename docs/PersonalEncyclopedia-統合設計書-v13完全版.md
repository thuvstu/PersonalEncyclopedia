...
**バージョン:** 13.0 (Complete Edition・実装経験統合版)
**作成日:** 2026-08-10
**ベース:** v11.0(Unified Edition) + 実装経験(Phase 0〜3完了・v12.0での撤回・報告書.mdの教訓)を統合
...
## v13.0での主な変更点(v11.0からの差分)

| 変更 | 内容 |
|---|---|
| §2.5(新設) | 実装原則・作業プロトコルを正式に設計書へ組み込み(v12.0撤回の報告書分析が根拠) |
| §3.4(新設) | 起動シーケンス・段階的初期化アーキテクチャを明文化 |
| §5.8(新設) | ホワイトボード・Wikipedia記事・カスタムフィールド・和暦マスタのテーブル設計を正式化 |
| §6 | バックアップ方式をSAF方式に確定、成功可視化の仕様を追加 |
| §7.6-7.7(新設) | スレッドセーフ設計・Ollama Provider実装の詳細化 |
| §8.8-8.9(新設) | FSRS詳細仕様・和暦マスタ運用方針の追加 |
| §10.1(新設) | Ktorルーティング分割アーキテクチャの追加 |
| §11.6-11.9(新設) | ホワイトボード画面・Wikipediaビルダー画面・リッチテキストエディタ・大規模画面分割方針の追加 |
| §12.6(新設) | 重複排除アーキテクチャの追加 |
| §13 | ロードマップをPhase 3完了実績とRound方式(1機能1ビルド)に全面更新 |
| §15 | nextPKM.txt(PWA/SQLite-WASM案)を不採用案として追加 |
...
### 7.7 Ollama Provider詳細(v13.0で具体化)

LAN内に自宅PC等でOllamaサーバーが稼働している場合、Gemini APIの代わりにこれを使う。DBを持たない純粋な推論エンドポイントであるため、「PC=閲覧専用・データ本体はAndroid」という原則には抵触しない(§0.1参照)。

```kotlin
class OllamaClient(private val baseUrl: String) : IAiProvider {
    override suspend fun generate(prompt: String, jsonMode: Boolean): String {
        val body = buildJsonObject {
            put("model", "llama3.1")   // 設定画面(§11.5 AI設定UI)で変更可能
            put("prompt", prompt)
            put("stream", false)
            if (jsonMode) put("format", "json")
        }
        return httpClient.post("$baseUrl/api/generate") { setBody(body) }.bodyAsJsonField("response")
    }
    // 埋め込みはOllamaの/api/embeddingsエンドポイントを利用可能だが、
    // Gemini Embeddingとの次元数の不一致(§7.1.1)に注意。
    // 混在させる場合はモデルごとに別のembeddingカラム(model名で判別)を持たせる必要がある
}

// providerの切替はDI(§9.2)のBrainModuleで行う。設定値はEncryptedSharedPreferencesに保存(§2.5準拠)
fun provideAiProvider(settings: Settings): IAiProvider =
    if (settings.ollamaBaseUrl != null) OllamaClient(settings.ollamaBaseUrl) else GeminiLlm()
```
...
*本書はこれまでの全7系統の学習支援システム計画(Supabase案・GAS/LearningSheet v25・LearningMasterMap・ALH Omni-Master v26/All-Specification v5.2・Personal Knowledge OS v10・KnOS EX・quizstudy/lumina_pkm/txt.md)に加え、実装で得た教訓(v12.0撤回の分析・報告書.md)、および検討の上で不採用とした2案(MyBase・nextPKM.txt)を統合したv13.0版である。ベースはAndroidネイティブ最新版であるPersonalEncyclopediaとし、その「データ主権をAndroidに置く」というエコで壊れにくい骨格は、実装の紆余曲折を経てなお変更していない。「何を作るか」はv11.0〜v12.0の設計を継承し、「どう作るか」は§2.5の実装原則によって刷新した。*
