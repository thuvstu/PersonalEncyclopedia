...
## ホワイトボードの「接続線」は connection ではない (walkthrough40)

ホワイトボード画面でカード同士を線で結べる(カード右下の 🔗 → 接続先のカードをタップ)。この線は `whiteboard_edge` テーブル(DB v11)に保存される**ボード固有の見た目の線**で、承認制の `connection` とは別物。

- 線を張っても `connection` / `connection_candidate` には**書き込まれない**(自動接続OFFの原則を白板でも守る)
- 型なしメモ(`whiteboard_note`)同士にも張れる。`connection` は entry 間にしか張れないのでここが違う
- 線の中点のチップをタップするとラベル編集・削除ができる。ノードを消すとそのノードの線も消える
- 「この線を connection に昇格する」導線は将来(エッジ → 候補作成 → 承認の順を想定)

## 自動リンク(閲覧時の下線)は connection ではない (walkthrough44/46)

詳細画面の本文・定義文と Wiki 記事では、他の entry のタイトルが自動でリンク化される(`AutoLinker`)。これは**閲覧時の装飾だけ**で、タップしてもプレビュー/遷移するのみ。`connection` にも `connection_candidate` にも書き込まない。

- 索引はタイトルに加えて、**定義の読み・人物の別名(「別名(カンマ区切り)」欄)と正式名・組織/場所/出来事の正式名**からも作られる(wt46)。「福澤諭吉」を登録して別名に「福沢諭吉」を書けば、本文中の「福沢諭吉」もリンクになる
- 全角/半角・大文字/小文字・半角カナ・語末の「ー」の有無は自動で吸収する。異体字(澤/沢)やひらがな⇄カタカナは吸収しないので、必要なら別名に書く
